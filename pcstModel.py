...
class pcstModel():
    def __init__(self, G, terminals = [1,2], weight = 'weight', minCapacity = 0, root = 1, pos = None, prize = 'prize'):
        self.G = G
        ...
        self.addConstraints(prize, weight, terminals)

    def addConstraints(self, prize, weight, terminals):
        # set objective: Maximize the profit
        self.m.setObjective(quicksum([self.G.G.nodes[node].get(prize, 0) * node_var for node, node_var in self.nodes.items()])
                    - quicksum([edge_var * self.G.G.edges[edge][weight] for edge, edge_var in self.edges.items()]),
                    GRB.MAXIMIZE)
        ...
        self.m.addConstr(gurobipy.quicksum(self.flows[edge] for edge in self.Neighbourhood if edge[0] == self.root) >= self.numTerminals)   
